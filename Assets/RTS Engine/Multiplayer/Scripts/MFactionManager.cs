...
namespace RTSEngine
{
	public class MFactionManager : NetworkBehaviour {
		...
		public float LockstepCycle = 0.4f; //the cycle's length, at which the player can send inputs.
		...
		int ReadyPlayers = 0;
        public List<Spawner> ReadySpawners = new List<Spawner>();
        public static float SendUnitTimer = 10f;
        private float sendUnits = SendUnitTimer;
...
*/
        ...
        public void ArenaTryToSpawnUnit(string UnitCode, Vector3 Pos, NetworkInstanceId Creator, int factionID)
        {
            if (isLocalPlayer)
            {
                CmdArenaSpawnUnit(UnitCode, Pos, Creator, factionID); //ask the server to spawn this new unit
            }
        }

        //spawn a unit from the server
        [Command]
        public void CmdArenaSpawnUnit(string UnitCode, Vector3 Pos, NetworkInstanceId Creator, int factionID)
        {
            int i = 0;
            GameObject UnitPrefab = null;
            //search for the unit's prefab:
            ...
            { //if the requested unit is valid:
              //create it in the server
                ...
                //send the spawned unit to the rally point:
                RpcArenaSetUnitCreator(UnitClone, Pos);
            }

        }

        [ClientRpc]
        public void RpcArenaSetUnitCreator(GameObject Unit, Vector3 Pos)
        {
            if (Unit.GetComponent<Unit>())
            {
                Unit.GetComponent<Unit>().gameObject.GetComponent<NavMeshAgent>().enabled = true;
                if (Unit.GetComponent<Unit>().FactionID == GameManager.PlayerFactionID)
                { //only if the local player controls this unit
                    GameObject Creator = ClientScene.FindLocalObject(Unit.GetComponent<Unit>().CreatedByID);
                    Unit.GetComponent<Unit>().SpawnedBy = Creator.GetComponent<Spawner>();
                }
            }

            Unit.transform.position = Pos;

            if (Unit.GetComponent<Unit>().SpawnedBy != null && GameManager.PlayerFactionID == Unit.GetComponent<Unit>().FactionID && isLocalPlayer == true)
            {
                //if the new unit does not have a task when spawned, send them to the goto position.
                Unit.GetComponent<Unit>().SpawnedBy.SendUnitToAttack(Unit.GetComponent<Unit>());
            }
        }
        ...
        public void TryToSpawnSpawner(string SpawnerCode, Vector3 Pos, float RotY)
        {
            if (isLocalPlayer)
            {
                CmdSpawnSpawner(SpawnerCode, Pos.x, Pos.y, Pos.z, RotY); //ask the server
            }
        }

        [Command]
        public void CmdSpawnSpawner(string SpawnerCode, float PosX, float PosY, float PosZ, float RotY)
        {
            int i = 0;
            GameObject SpawnerPrefab = null;
            while (SpawnerPrefab == null && i < NetworkMgr.spawnPrefabs.Count)
            {
                if (NetworkMgr.
                    spawnPrefabs[i].
                    GetComponent<Spawner>())
                {
                    if (NetworkMgr.spawnPrefabs[i].GetComponent<Spawner>().Code == SpawnerCode)
                    {
                        SpawnerPrefab = NetworkMgr.spawnPrefabs[i];
                    }
                }
                i++;
            }
            if (SpawnerPrefab != null)
            { //if the building to spawn is valid:

                //create the building's object
                GameObject BuildingClone = Instantiate(SpawnerPrefab);

                //set its position and faction:
                BuildingClone.transform.position = new Vector3(PosX, PosY, PosZ);
                BuildingClone.GetComponent<Spawner>().FactionID = FactionID;
                BuildingClone.GetComponent<Spawner>().Placed = true;

                //spawn the building for all clients:
                NetworkServer.SpawnWithClientAuthority(BuildingClone, connectionToClient);
                RpcRotateBuilding(BuildingClone, RotY);
            }
        }
        [ClientRpc]
        public void RpcRotateBuilding(GameObject Obj, float RotY)
        {
            if (Obj.GetComponent<Spawner>())
                Obj.GetComponent<Spawner>().Rotate(RotY);
            if(isLocalPlayer)
                ReadySpawners.Add(Obj.GetComponent<Spawner>());
        }
	...
	public void CmdSyncBuildingHealth (NetworkInstanceId BuildingNetID, float Value)
	{
        ...
        public void SpawnUnitsOnBattlefield()
        {
