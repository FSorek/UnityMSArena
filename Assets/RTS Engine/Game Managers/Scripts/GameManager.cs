...
namespace RTSEngine
{
	public class GameManager : MonoBehaviour {
        ...
        [HideInInspector]
        public RTSArena.SpawnerPlacement spawnerPlacement;
		...
		void Awake () 
		{
            ...
            spawnerPlacement = FindObjectOfType(typeof(RTSArena.SpawnerPlacement)) as RTSArena.SpawnerPlacement;
			...
			if (NetworkMgr != null) { //If there's actually a network map manager, it means that the map was loaded from the multiplayer menu, meaning that this is a MP game.
				...
				if (MFactionsInfo.Length <= Factions.Count) {
					...
					/*for (int i = 0; i < Factions.Count; i++) {
                        var obj = Factions[i].CapitalBuilding.gameObject;
